...
class Gen_CPG(ImbAlgorithmBase):
    def __init__(self, args, net_builder, tb_log=None, logger=None):
        ...
        # augment r and dim
        self.sim_num = None
        self.feat_aug_r = None # feature augmentation ratio
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            # self.warm_up select unlabeled data but still use labeled data only to compute loss
                ...
                self.optim_cfc = feature_mean_center # (num_classes, num_features)
    ...
    def train_step(self, idx_lb, x_lb_w, x_lb_s, y_lb, y_lb_noised, idx_ulb, x_ulb_w, x_ulb_s, y_ulb):
        ...
        with self.amp_cm():
            ...
            if self.epoch < self.warm_up:
                ...
                aug_lbs_x_lb = lb
                aug_feats_x_lb = feats_x_lb_w # (b, feat_dim)
                ...
                for feat, label, aug_time in zip(feats_x_lb_w[aug_feats_x_lb_times != 0], lb[aug_feats_x_lb_times != 0], aug_feats_x_lb_times[aug_feats_x_lb_times != 0]):
                    repeated_feat = torch.cat([feat.unsqueeze(0) for _ in range(aug_time)], dim=0) # (aug_time, feat_dim)
                    repeated_label = torch.cat([label.unsqueeze(0) for _ in range(aug_time)], dim=0) # (aug_time, num_classes)
                    repeated_feat_aug_r = (torch.cat([self.feat_aug_r[torch.argmax(label).item()].unsqueeze(0) for _ in range(aug_time)], dim=0)).view(-1, 1) # (aug_time, 1)
                    repeated_feat_norm = torch.cat([F.normalize(feat.unsqueeze(0)) for _ in range(aug_time)], dim=0) # (aug_time, feat_dim)
                ...
                sup_loss = self.ce_loss(aug_logits_x_lb, aug_lbs_x_lb, reduction='mean')
