...
class WideResNet(nn.Module):
    def __init__(self, first_stride, num_classes, depth=28, widen_factor=2, drop_rate=0.0, **kwargs):
        ...
        self.feat_proj = nn.Sequential(
            nn.Linear(channels[3], self.emb_dim, bias=True),
            nn.LeakyReLU(negative_slope=0.1, inplace=True)
        )
        self.classifier = nn.Linear(self.emb_dim, num_classes)
        
        # aux_classifier
        self.aux_classifier = nn.Linear(self.emb_dim, num_classes)

        # rot_classifier
        self.rot_classifier = nn.Linear(self.emb_dim, 4)
    ...
    def forward(self, x, only_fc=False, only_feat=False, **kwargs):
        ...
        feat64 = self.feat_proj(out)

        if only_feat:
            return feat64
        
        output = self.classifier(feat64)
        rot_output = self.rot_classifier(feat64)
        aux_output = self.aux_classifier(feat64)
        result_dict = {'rot_logits':rot_output, 'aux_logits':aux_output, 'logits':output, 'feat':feat64}
        return result_dict
